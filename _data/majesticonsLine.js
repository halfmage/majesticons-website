  ...
  return new Promise(async (resolve, reject) => {
    ...
    const iconData = await Promise.all(
      svgIcons.map(async (name) => {
        ...
        const sourceWithClass = source.replace('<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"', '<svg class="w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"');
        return {
          name,
          source,
          namePlain,
